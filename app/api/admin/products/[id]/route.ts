...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const { name, slug, description, price, compare_at_price, category_id,
            status, featured, sku, tags, variants, images } = body;
    ...
    const { error: productError } = await supabase
      .from('products')
      .update({
        name, slug, description, price, compare_at_price: compare_at_price || null,
        category_id: category_id || null, status, featured, sku: sku || null,
    ...
    if (variants) {
      ...
      if (variants.length > 0) {
        const variantRows = variants.map((v: { size: string; color: string; stock_quantity: number; sku: string }) => ({
          product_id: id, size: v.size, color: v.color || null,
    ...
    if (images) {
      ...
      if (images.length > 0) {
        const imageRows = images.map((img: { url: string; is_primary: boolean; sort_order: number }, i: number) => ({
          product_id: id, url: img.url, is_primary: img.is_primary || i === 0,
